...
let
  ...
  # Desktop-only: relies on Hyprland's hyprctl, which doesn't exist on the
  # Termux phone (aarch64). Gated on aliyss.isPhone.
  herdrLaunch = lib.mkIf (!config.aliyss.isPhone) (
    let
      hyprctl = "${hyprland.packages.${pkgs.stdenv.hostPlatform.system}.hyprland}/bin/hyprctl";
    in
    pkgs.writeShellScriptBin "herdr-launch" ''
        set -euo pipefail
