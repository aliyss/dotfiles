...
** Herdr
*Configuration:* [[./flake/home-manager/apps/herdr.nix][herdr.nix]]
Fish wrapper helper: `herdr-workspace "main"` (defined in the same file). It starts the per-user herdr daemon on demand and lands the new shell on the `main` workspace — what every desktop pane and every SSH-from-phone session runs.

** Phone Access
*Guide:* [[./phone-remote-access.md][phone-remote-access.md]]

The desktop runs one per-user herdr daemon. Every interactive fish —
desktop panes, every fresh SSH session from the phone — execs
`herdr-workspace "main"` and becomes a herdr client on that same daemon.
So when the phone `ssh -p 22922`'s in, it sees the same workspace,
panes, and agents the desktop sees. No tmux wrap; herdr's own daemon
does the equivalent (and the herdr docs explicitly say to install any
SSH client on the phone and run `herdr` there).

*Rebuild order after editing this repo:*

#+begin_src bash
  update-home                     # fish + herdr-workspace hookup
  sudo update-system -s bequitta  # openssh + firewall (rebases HM if needed)
#+end_src
...
*** Styling
Need to add additional styling for what information to hide and what to show.
